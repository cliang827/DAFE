...
for qt = 1:tot_query_times
    ...
    for i=1:length(log_alpha_set)
        for j=1:length(beta_set)
            alpha_beta_auc(i,j) = mean(auc_f(log_alpha==log_alpha_set(i) & beta==beta_set(j),qt));
        end
    end
    ...
    ylabel('log(\alpha)');xlabel('\beta%');zlabel('auc');
    set(gca,'xticklabel',num2cell(beta_set));
    set(gca,'yticklabel',num2cell(log_alpha_set));
    ...
    for i=1:length(log_alpha_set)
        for j=1:length(log_gamma_set)
            alpha_gamma_auc(i,j) = mean(auc_f(log_alpha==log_alpha_set(i) & log_gamma==log_gamma_set(j),qt));
        end
    end
    ...
    for i=1:length(beta_set)
        for j=1:length(log_gamma_set)
            beta_gamma_auc(i,j) = mean(auc_f(beta==beta_set(i) & log_gamma==log_gamma_set(j),qt));
        end
    end
