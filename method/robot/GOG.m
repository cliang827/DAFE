...
xlefts = max( cols + 1 - halffsize, 2); % here 2 is for the zero padding row/column used in create_IH_MT
xrights = min( cols + 1 + halffsize, size(IH_Corr, 2));
yups = max( rows + 1 - halffsize, 2);   % here 2 is for the zero padding row/column used in create_IH_MT
...
points2 = (xlefts-2) *size(IH_Corr, 1) + yups-1;    % x-1, y-1
points3 = (xrights-1)*size(IH_Corr, 1) + yups-1;    % x,   y-1
points4 = (xlefts-2)*size(IH_Corr, 1) + ydowns;     % x-1, y
...
% change the grid parameter's measuing unit from pixels to patches
